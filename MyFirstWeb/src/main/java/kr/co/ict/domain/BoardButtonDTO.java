...
// 원래 VO(value Object)와 DTO(Data Transfer Object)는 큰 차이를 두지 않고 사용한다.
// 굳이 차이를 두자면 VO는 DB에서 가져온 자료를 그대로 전달 할 때,
// DTO는 DB에서 꺼내온 데이터가 가공되어 전달할 때 쓰는것을 지칭한다.
// 다만 DTO, VO는 엄격하게 구분되지 않고 프로그래머에 따라서는
// 그냥 DTO를 VO와 동일하게 쓰는 경우도 있으므로
// 사용하는 사람이 부르고 싶은대로 불러도 무방하다.
public class BoardButtonDTO {
	private int boardCount; // 전체 글 개수
	private int currentPage; // 현재 조회 중인 페이지
	...
	public BoardButtonDTO(int boardCount, int currentPage) {
		 // 총 글 개수와, 현재 조회중인 페이지 정보를 입력받아 아래에 딸릴 버튼개수, 현재 깔아야 하는 번호대의 버튼을 구한다.
		// boardCount, currentPage를 멤버변수에 저장하기
